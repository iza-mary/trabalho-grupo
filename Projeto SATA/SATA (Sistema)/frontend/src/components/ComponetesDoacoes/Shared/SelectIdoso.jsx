...
function SelectIdoso({ setIdoso, setErrors, setValidated, errors, selectedIdosoEdit, errorKey = "idoso" }) {
  ...
  const fetchIdosos = async (nome) => {
    try {
      const todos = await idosoService.getAll();
      const termo = (nome || '').toLowerCase();
      const filtrados = (todos || []).filter((i) => (i?.nome || '').toLowerCase().includes(termo));
      setListaIdosos(filtrados);
      if (filtrados.length === 0 && termo.length > 0) {
      ...
      console.error('Erro ao buscar idosos:', err?.message || err);
      setListaIdosos([]);
      setErrors?.((prev) => ({ ...prev, [errorKey]: 'Falha ao carregar idosos' }));
      setValidated?.(false);
    }
  };
