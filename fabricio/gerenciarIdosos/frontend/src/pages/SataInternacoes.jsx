            ...
            <Col md={4}>
              <Card className="stats-card">
                <Card.Body className="text-center">
                  <h5 className="mb-1">{internacoesFiltradas.length}</h5>
                  <small className="text-muted">
                    {filtroStatus === 'ativas' ? 'Internações Ativas' : 
                     filtroStatus === 'finalizadas' ? 'Internações Finalizadas' : 
          ...
          {internacoesFiltradas.length === 0 ? (
            ...
            <Row>
              {internacoesFiltradas.map((internacao) => (
                <Col key={internacao.id} lg={6} xl={4} className="mb-4">
                  <Card className="internacao-card h-100">
                    <Card.Body>
                      <div className="d-flex justify-content-between align-items-start mb-3">
                        <div className="d-flex align-items-center">
                          <PersonFill className="me-2" />
                          <strong>{obterNomeIdoso(internacao.idoso_id)}</strong>
                        </div>
                        <Badge 
                          bg={internacao.status === 'ativa' ? 'success' : 'secondary'}
                      ...
                      <div className="internacao-info">
                        <div className="info-item">
                          <GeoAlt className="me-2 text-primary" />
                          <span><strong>Quarto:</strong> {obterNumeroQuarto(internacao.quarto_id)} - <strong>Cama:</strong> {internacao.cama}</span>
                        </div>
                        <div className="info-item">
                          <CalendarEvent className="me-2 text-info" />
                          <span><strong>Entrada:</strong> {new Date(internacao.data_entrada).toLocaleDateString('pt-BR')}</span>
                        </div>
                        {internacao.status === 'ativa' && (
                          <div className="info-item">
                            <Clock className="me-2 text-warning" />
                            <span><strong>Dias internado:</strong> {calcularDiasInternacao(internacao.data_entrada)}</span>
                          </div>
                        )}
                        {internacao.data_saida && (
                          <div className="info-item">
                            <CalendarEvent className="me-2 text-secondary" />
                            <span><strong>Saída:</strong> {new Date(internacao.data_saida).toLocaleDateString('pt-BR')}</span>
                          </div>
                        )}
                      </div>
